    ...
    def gameOfLife(self, board: List[List[int]]) -> None:
        ...
        for i, row in enumerate(board):
            for j, value in enumerate(row):
                live_c = 0
                for x,y in around:#count surrending cells
                    newx,newy = x+i, y+j
                    if m>newx>=0 and n>newy>=0:#legit
...
# @lc code=end
# inplace 
# 0->1 1->0 will change the live count
# use special marks to memorize these conditions
