...
class Solution:
    def totalNQueens(self, n: int) -> int:
        self.res = 0#member variable

        def helper(level=0, col=set(), left=set(), right=set()):
            if level==n:
                return True
            for j in range(n):
                if j not in col and level-j not in right and level+j not in left:
                    if helper(level+1, col|{j}, left|{level+j}, right|{level-j}):
                        self.res += 1
            return False
        ...
        return self.res
# @lc code=end
# variable is not usable in inner func
